...
def label_axes(axes, textpos, color="k", uppercase=False, bracket=True):
    """
    ...
    - color: string or array. Passes as a parameter to all text
    ...
    #Get the selected color for everybody
    if type(color) is str:
        color = [color for i in range(len(axes))]
    
    #Iterate over the axes and set the things
    if type(axes) is np.ndarray:
        for i,ax in enumerate(axes.flatten()):
            ax.text(textpos[i][0], textpos[i][1], axlabel.format(alphabet[i]), color=color[i], transform=ax.transAxes, weight="bold")
    elif type(axes) is list:
