...
def example_tuning_curve(ax, rates, rates_err, id):
    ...
    rangle = rates[id, :]
    rangle_err = rates_err[id, :]

    ax.annotate("", xytext=(4, 4), xy=(4, 2.75), arrowprops=dict(arrowstyle="->"), color='k')
    ax.plot(np.arange(8), rangle,  lw=1, color='dimgray')
    ax.plot(np.arange(8), rangle,  lw=1, color='dimgray', ls='none', marker='o', ms=cr.ms)
    ax.errorbar(np.arange(8), rangle,  yerr = rangle_err , color='dimgray', fmt='none')
...
def plot_currents(axes, units, rates, vij):
    ...
    for i,layer in enumerate(['L23', 'L4', "Total"]):
        ...
        avgcurrent[layer] = avgcurrent[layer] / maxcur
        stdcurrent[layer] = stdcurrent[layer] / maxcur
    ...
    for ax in axes:
        ...
        ax.set_xlabel(r"$k - \hat k_\text{post}$")
...
def plot_error_prediction_dist(ax, signed_delta):
    ...
    for layer in ['L23', 'L4', 'Total']:
        ...
        ax.step(sfq, dist_diffs, color=cr.lcolor[layer])
    
    ax.set_xticks([-1, 7, 16], ['-0.17', '0.0', '0.17'])
    ax.set_xlabel(r"$\hat k_\text{pred} - \hat k$")
